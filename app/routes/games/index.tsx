...
function NewTable() {
  ...
  return (
    <Link className="rounded bg-teal-9 p-2 text-sage-12" to="/games/new-table">
      <PlusIcon />
    </Link>
  );
}
    ...
    <div className="">
      <div className="grid grid-cols-1 gap-6 py-5 sm:grid-cols-2">
        {games.map((game) => {
          return (
            <article
              className="cursor-pointer rounded border border-sage-7 px-4 py-2 transition ease-in-out hover:scale-105 hover:border-2 hover:border-teal-9"
              key={game.id}
            >
              <Link to={game.id}>
